            ...
            for n in range(len(personwiseKeypoints)):
                for i in range(18):
                    if (personwiseKeypoints[n][i] == -1):
                        ...
                        if ((len(detected_keypoints[i]) != 0) and len(detected_keypoints[i]) > n and
                                personwiseKeypoints[n][i] == detected_keypoints[i][n][3]):
                            finalPeoplePoint.append(
                                [detected_keypoints[i][n][0], detected_keypoints[i][n][1], detected_keypoints[i][n][2],
                                 i])
                ...
                for j in range(len(detected_keypoints[i])):
                    ...
                    if i==0 or i == 2 or i == 5 or i == 8 or i == 11 or i == 16 or i == 17:
                      ...
                      if (status == "Right Pose" or status =="normal head alignment" ):
                        colors[i]= green
                        cv2.circle(frame, detected_keypoints[i][j][0:2], 7, black, -1, cv2.LINE_AA)
